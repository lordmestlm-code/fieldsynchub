...
function seedData() {
  ...
  const customers = [
    ...
    { id: 'cust-2', name: 'Sarah Johnson', email: 'sarah.j@email.com', phone: '(555) 234-5678', address: '456 Maple Avenue', city: 'Springfield', state: 'IL', zipCode: '62702', createdAt: '2024-02-20T14:30:00Z', totalJobs: 8, totalSpent: 2100.00 },
    ...
    { id: 'cust-4', name: 'Emily Davis', email: 'emily.d@email.com', phone: '(555) 456-7890', address: '321 Elm Street', city: 'Springfield', state: 'IL', zipCode: '62703', createdAt: '2024-04-05T11:45:00Z', totalJobs: 5, totalSpent: 890.00 },
  ...
  // Seed Jobs
  const jobs = [
    { id: 'job-1', title: 'AC Unit Installation', description: 'Install new central AC unit in residential home', customerId: 'cust-1', technicianId: 'tech-1', status: 'scheduled', priority: 'high', scheduledDate: '2026-03-14', scheduledTime: '09:00', estimatedDuration: 240, address: '123 Oak Street, Springfield, IL 62701', notes: 'Customer will be home. Pre-install survey completed.', price: 2500.00, createdAt: '2024-03-01T10:00:00Z', updatedAt: '2024-03-10T14:30:00Z' },
    { id: 'job-2', title: 'Leak Repair', description: 'Fix leaking pipe under kitchen sink', customerId: 'cust-2', technicianId: 'tech-2', status: 'in_progress', priority: 'urgent', scheduledDate: '2026-03-14', scheduledTime: '14:00', estimatedDuration: 90, address: '456 Maple Avenue, Springfield, IL 62702', price: 350.00, createdAt: '2024-03-05T09:00:00Z', updatedAt: '2024-03-14T08:30:00Z' },
    { id: 'job-3', title: 'Annual Furnace Maintenance', description: 'Yearly furnace inspection and maintenance', customerId: 'cust-3', technicianId: 'tech-1', status: 'pending', priority: 'medium', scheduledDate: '2026-03-15', scheduledTime: '10:00', estimatedDuration: 120, address: '789 Pine Road, Chicago, IL 60601', price: 199.00, createdAt: '2024-03-08T11:00:00Z', updatedAt: '2024-03-08T11:00:00Z' },
    { id: 'job-4', title: 'Dishwasher Installation', description: 'Install new dishwasher and connect to plumbing', customerId: 'cust-4', status: 'pending', priority: 'medium', scheduledDate: '2026-03-16', scheduledTime: '11:00', estimatedDuration: 180, address: '321 Elm Street, Springfield, IL 62703', price: 450.00, createdAt: '2024-03-10T14:00:00Z', updatedAt: '2024-03-10T14:00:00Z' },
...
app.get('/api/customers/:id', (req, res) => {
  ...
  const customers = queryToObjects(result);
...
app.post('/api/customers', (req, res) => {
  ...
  db.run("INSERT INTO customers (id, name, email, phone, address, city, state, zipCode, notes, createdAt, totalJobs, totalSpent) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0)",
    [id, name, email, phone, address, city, state, zipCode, notes, createdAt]);
  saveDB();
...
app.put('/api/customers/:id', (req, res) => {
  ...
  db.run("UPDATE customers SET name = ?, email = ?, phone = ?, address = ?, city = ?, state = ?, zipCode = ?, notes = ? WHERE id = ?",
    [name, email, phone, address, city, state, zipCode, notes, req.params.id]);
  saveDB();
...
app.get('/api/technicians/:id', (req, res) => {
  const result = db.exec("SELECT * FROM technicians WHERE id = ?", [req.params.id]);
  const techs = queryToObjects(result);
  res.json(techs[0] || null);
});
...
app.post('/api/technicians', (req, res) => {
  ...
  db.run("INSERT INTO technicians (id, name, email, phone, specialty, status, rating, jobsCompleted, color) VALUES (?, ?, ?, ?, ?, ?, 5.0, 0, ?)",
    [id, name, email, phone, specialty, status || 'available', color]);
  saveDB();
  res.json({ id, name, email, phone, specialty, status, color });
});
...
app.put('/api/technicians/:id', (req, res) => {
  ...
  db.run("UPDATE technicians SET name = ?, email = ?, phone = ?, specialty = ?, status = ?, color = ? WHERE id = ?",
    [name, email, phone, specialty, status, color, req.params.id]);
  saveDB();
  res.json({ success: true });
});
...
app.get('/api/jobs/:id', (req, res) => {
  const result = db.exec("SELECT * FROM jobs WHERE id = ?", [req.params.id]);
  const jobs = queryToObjects(result);
...
app.post('/api/jobs', (req, res) => {
  ...
  db.run("INSERT INTO jobs (id, title, description, customerId, technicianId, status, priority, scheduledDate, scheduledTime, estimatedDuration, address, notes, price, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [id, title, description, customerId, technicianId, status || 'pending', priority || 'medium', scheduledDate, scheduledTime, estimatedDuration, address, notes, price || 0, createdAt, updatedAt]);
  saveDB();
...
app.put('/api/jobs/:id', (req, res) => {
  ...
  db.run("UPDATE jobs SET title = ?, description = ?, customerId = ?, technicianId = ?, status = ?, priority = ?, scheduledDate = ?, scheduledTime = ?, estimatedDuration = ?, actualDuration = ?, address = ?, notes = ?, price = ?, updatedAt = ? WHERE id = ?",
    [title, description, customerId, technicianId, status, priority, scheduledDate, scheduledTime, estimatedDuration, actualDuration, address, notes, price, updatedAt, req.params.id]);
  saveDB();
  res.json({ success: true });
});
...
app.post('/api/invoices', (req, res) => {
  ...
  db.run("INSERT INTO invoices (id, invoiceNumber, customerId, jobId, status, items, subtotal, tax, total, dueDate, notes, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [id, invoiceNumber, customerId, jobId, status || 'draft', JSON.stringify(items), subtotal, tax, total, dueDate, notes, createdAt, createdAt]);
  saveDB();
...
app.put('/api/invoices/:id', (req, res) => {
  ...
  db.run("UPDATE invoices SET status = ?, paidDate = ?, updatedAt = ? WHERE id = ?",
    [status, paidDate, updatedAt, req.params.id]);
  saveDB();
  res.json({ success: true });
});
...
app.post('/api/estimates', (req, res) => {
  ...
  db.run("INSERT INTO estimates (id, estimateNumber, customerId, jobId, status, items, subtotal, tax, total, validUntil, notes, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [id, estimateNumber, customerId, jobId, status || 'draft', JSON.stringify(items), subtotal, tax, total, validUntil, notes, createdAt, createdAt]);
  saveDB();
